...
const GI_SOURCE = 'sc';
const GI_TITLE = 'Genetic Interaction Classifier - Single Cell';
  ...
  try {
    ...
    if ($ajax === 'options') {
      $tissues = $pdo->query("
        ...
        FROM gi_result
        WHERE source='sc' AND TRIM(COALESCE(meta_biosample_tissue_name,''))<>''
        ORDER BY meta_biosample_tissue_name
      ")->fetchAll(PDO::FETCH_COLUMN) ?: [];
      $cellTypes = $pdo->query("
        SELECT DISTINCT " . celltype_sql('g') . " AS cell_type
        FROM gi_result g
        WHERE g.source='sc' AND TRIM(COALESCE(" . celltype_sql('g') . ",''))<>''
        ORDER BY cell_type
      ")->fetchAll(PDO::FETCH_COLUMN) ?: [];
      $perturbs = $pdo->query("
    ...
    if ($ajax === 'linked_options') {
      ...
      $params = [':q' => '%' . $qSpecies . '%'];
      $where = ["g.source='sc'", "TRIM(COALESCE(" . species_sql('g') . ",''))<>''", "LOWER(" . species_sql('g') . ") LIKE LOWER(:q)"];
      ...
      $params = [':q' => '%' . $qTissue . '%'];
      $where = ["g.source='sc'", "TRIM(COALESCE(g.meta_biosample_tissue_name,''))<>''", "LOWER(g.meta_biosample_tissue_name) LIKE LOWER(:q)"];
      ...
      $params = [':q' => '%' . $qCell . '%'];
      $where = ["g.source='sc'", "TRIM(COALESCE(" . celltype_sql('g') . ",''))<>''", "LOWER(" . celltype_sql('g') . ") LIKE LOWER(:q)"];
      if ($speciesSel) $where[] = species_sql('g') . " IN (" . $buildIn($speciesSel, 's', $params) . ")";
      if ($tissuesSel) $where[] = "g.meta_biosample_tissue_name IN (" . $buildIn($tissuesSel, 't', $params) . ")";
      if ($perturbsSel) $where[] = "g.target_gene_name IN (" . $buildIn($perturbsSel, 'p', $params) . ")";
      ...
      $params = [':q' => '%' . $qPerturb . '%'];
      $where = ["g.source='sc'", "TRIM(COALESCE(g.target_gene_name,''))<>''", "LOWER(g.target_gene_name) LIKE LOWER(:q)"];
      if ($speciesSel) $where[] = species_sql('g') . " IN (" . $buildIn($speciesSel, 's', $params) . ")";
      if ($tissuesSel) $where[] = "g.meta_biosample_tissue_name IN (" . $buildIn($tissuesSel, 't', $params) . ")";
      if ($cellTypesSel) $where[] = celltype_sql('g') . " IN (" . $buildIn($cellTypesSel, 'c', $params) . ")";
    ...
    if ($ajax === 'perturb_options') {
      $tissues = parse_param_list('tissues');
      $cellTypes = parse_param_list('cell_types');
      $where = ["g.source='sc'", "TRIM(COALESCE(g.target_gene_name,''))<>''"];
      $params = [];
      if ($tissues) {
        $ph = [];
...
<main class="layout-page gi-main pb-5">
  <div class="container-fluid page-shell pt-2">
    <div class="mb-3">
      ...
      <div class="muted-small">This tool identifies genetic interaction subtype in user-selected single cell dataset.</div>
    </div>
...
const ajaxEndpoint = window.location.pathname.split('/').pop() || 'sc_gi_tool.php';
...
async function runPage(page){
  ...
  const qs = new URLSearchParams();
  qs.set('ajax','query');
  // Use compact CSV params for query to avoid max_input_vars truncation on large selections.
  // parse_param_list() on PHP side supports comma-separated strings.
  qs.set('species', selectedArray('species').join(','));
  qs.set('tissues', selectedArray('tissue').join(','));
  qs.set('cell_types', selectedArray('cell').join(','));
  qs.set('perturbations', selectedArray('perturb').join(','));
